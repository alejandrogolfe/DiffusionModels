...
def validate_context_class_config(condition_config):
    assert 'context_class_config' in condition_config, \
        "Class conditioning desired but class condition config missing"
    assert 'num_classes' in condition_config['context_class_config'], \
        "num_class missing in class condition config"
